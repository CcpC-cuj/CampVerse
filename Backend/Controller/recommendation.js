...
async function getEventRecommendations(req, res) {
  try {
    ...
    const userProfile = {
      ...
      skills: user.skills || [],
      attendedEvents: userHistory.filter(log => log.status === 'attended').map(log => ({
      ...
      registeredEvents: userHistory.filter(log => log.status === 'registered').map(log => ({
        eventId: log.eventId._id,
        title: log.eventId.title,
        type: log.eventId.type,
        tags: log.eventId.tags || [],
    ...
    }).select('title description tags type organizer schedule hostUserId institutionId');
    ...
    try {
      const mlResponse = await axios.post(`${ML_API_URL}/recommend`, recommendationData, {
        timeout: 10000
      });
      ...
      const finalRecommendations = recommendations.map(rec => {
        const event = recommendedEvents.find(e => e._id.toString() === rec.eventId);
        return {
          eventId: rec.eventId,
      ...
      const fallbackRecommendations = await getFallbackRecommendations(userProfile, availableEvents, limit);
    ...
    return res.status(500).json({ error: 'Error getting event recommendations' });
  }
}
...
async function getFallbackRecommendations(userProfile, availableEvents, limit) {
  ...
  const scoredEvents = availableEvents.map(event => {
    ...
    const attendedOrganizers = userProfile.attendedEvents.map(e => e.organizer);
    if (attendedOrganizers.includes(event.organizer)) score += 1;
    
    return {
      eventId: event._id,
      similarityScore: score,
      reason: score > 0 ? 'Based on your interests and skills' : 'Popular events',
      event: {
        _id: event._id,
        title: event.title,
        description: event.description,
