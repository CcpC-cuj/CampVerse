...
const UnifiedSidebar = ({ onDiscoverClick }) => {
  const { user, logout } = useAuth();
  const [institutionName, setInstitutionName] = useState('');
  const [institutionVerified, setInstitutionVerified] = useState(false);
  
  // Determine user role (adjust based on your user object structure)
  const isHost = user?.role === 'host' || user?.role === 'organizer';
  ...
  // Dynamic role text
  const roleText = isHost ? "Event Organizer" : "Student";
  const defaultInterests = isHost 
    ...
    <div className="h-screen w-64 flex flex-col bg-[#0b0f2b] border-r border-gray-800 text-white font-poppins overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto custom-scroll">
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <div className="text-xs text-gray-400">
                {roleText} • {collegeText}
              </div>
            </div>
          </div>
          <div className="mt-3 flex flex-wrap gap-1">
            {(user?.interests || defaultInterests).slice(0,6).map((tag, idx) => (
              <span
                key={idx}
        ...
        <div className="py-2">
          ...
          {/* Host-only: Manage Events */}
          {isHost && (
            <SidebarLink 
              icon="ri-calendar-event-line" 
              to="/host/manage-events" 
              label="Manage Events" 
            />
          )}
      ...
      <div className="p-4 border-t border-gray-700">
        <SidebarLink 
          ...
          to={isHost ? "/host/settings" : "/settings"} 
          label="Settings" 
        />
        <SidebarLink 
